...
def get_nebula_package_and_plugin(sensor_make):
    if sensor_make == "Ouster":
        return "nebula_ouster", "nebula::ros::OusterRosWrapper"
    return "nebula_ros", sensor_make + "RosWrapper"


def get_driver_input_topic(sensor_make):
    if sensor_make == "Ouster":
        return "points"
...
def launch_setup(context, *args, **kwargs):
    ...
    nebula_pkg, nebula_plugin = get_nebula_package_and_plugin(sensor_make)
    driver_input_topic = get_driver_input_topic(sensor_make)
    ...
    if sensor_make != "Ouster":
        driver_parameters[1]["calibration_file"] = sensor_calib_fp
    ...
    if LaunchConfiguration("launch_driver_only").perform(context).lower() == "true":
        ...
        nodes.append(
            ComposableNode(
                package=nebula_pkg,
                plugin=nebula_plugin,
                name=sensor_make.lower() + "_ros_wrapper_node",
                parameters=driver_parameters,
                remappings=[
                    (driver_input_topic, "pointcloud_raw_ex"),
                ],
